...
public class ProductRepository : IProductRepository
{
    ...
    {
        // verificar si el nombre es null o estas vacio y cantidad no puede ser menor o igual a cero
    ...
    {
        // si no nos manda el producto false
        if (product == null)
        {
            return false;
        }
    ...
    public ICollection<Product> SearchProducts(string searchTerm)
    {
        ...
        if (!string.IsNullOrEmpty(searchTerm))
        {
            query = query.Include(p => p.Category)
                    .Where(p => p.Name.ToLower().Trim().Contains(searchTermLower) || p.Description.ToLower().Trim().Contains(searchTermLower));
        }
